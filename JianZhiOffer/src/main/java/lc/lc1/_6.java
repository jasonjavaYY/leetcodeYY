...
* */
//字符串 将字符串s根据行数numRows从上往下、从左到右Z形排，输入"PAYPALISHIRING"行数3时如下：
//P   A   H   N
//A P L S I I G
//Y   I   R
public class _6 {
    public String convert(String s, int numRows) {
        ...
        if (r == 1 || r >= n) return s; //如果只排一行或排的行数超过字符串长度，返回原串
        int t = r * 2 - 2; //t=2r-2
        int c = (n + t - 1) / t * (r - 1);  //c列，c=(n+t-1)/t*(r−1)
        char[][] mat = new char[r][c];  //构造存放数组，r行c列
        for (int i = 0, x = 0, y = 0; i < n; ++i) {
            mat[x][y] = s.charAt(i); //遍历原字符数组每个字符
            if (i % t < r - 1) { //i mod t<r−1，向下移动
                ++x;
            } else {  //否则向右上移动
                --x; ++y;
            }
        ...
        for (char[] row : mat) { //遍历每一行，不是空字符就拼接到ans
            for (char ch : row) {
                if (ch != 0) {
                    ans.append(ch);
                }
